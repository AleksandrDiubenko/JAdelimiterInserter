...
(
    ...
    (が(?!(して|った|ら|かり))|か(?!([はもらなえがけげせぜてでねめれいきぎしちにんをうくぐすつぬむるりっ]|った|さ))|か[は]|は(?!ず)|も(?![のろ])|の(?![みにがはた為よ])|なく(?!て)|な(?![くのんらるいし])|する(?![なのよ])|から(?!して)|まで|に(?!([はもね]|ついて|よって|よる|関して|すら))|
    ...
    より(?=ずっと)|はじめて|[てで](?=くれ)|くなって(?!は)|され[るた](?![んの])|かった(?![んのりわっがぞよぜ])|もなくて(?!は)|あらゆる|すべて(の|を|では|じゃ(?!あ))|すぐに[はも]|すぐに(?![はも])|
    もなく(?!て)|ながら|がてら|った(?![らんのりわっがぞよぜ])|よりも|かも(?=[しれ])|とともに(?![はも])|と共に(?![はも])|もっとも|すべて(?![でのを])|ただの|まま(?=(\p{Han}|(?P=Kata)|[0-9０-９]))|
