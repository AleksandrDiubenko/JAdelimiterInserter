...
(
    (\p{Han}{1,2}|\p{Katakana}{2,12}|こと|ところ|\p{Han}(?:\p{Hiragana}(?!で))+\p{Han}|\p{Katakana}{2,12}\p{Han}|もの|入り|」|たち|ここ|そこ|\p{Han}ら|(?P<double>\p{Hiragana}{2})(?P=double)|[えけげせぜてでねめれ]る|まま|[あこそ]いつ|あ[なん]た|さん|まみれ|おそらく|たっぷり|気持ち|すら|さすが|くず|あちこち|もと|さま|[こそあど]れ|ど[れん]だけ|みんな|やつ|すで|だ|[こそあ]ちら|[こそあ]っち|[あわ]たく?し|みたい|どこ|[0-9０-９][%％年月日個本人枚匹頭羽冊台隻つ]?|かり|\p{Han}[いきしちにひみり])
    ...
    すなわち|[うくぐすつぬふむる]の[にはもがをでよ]|を|んな[のに]|[って]たら|として(?!も)|つまり|ちょっと|ちょうど|々な|々に(?![もは])|々に[もは]|たい(?=\p{Han})|けど|よう[なに]?(?=(\p{Han}{2}|\p{Katakana}{2}))|
    [ただ]と(?!(は|えば))|[ただ]とは|とは|[のただ]ほうが|ないほうが|[のただ]方が|ない方が|風に|[いきしちにひみり]たくて|[うくすつぬふむる]まて|[^一-龯]続く|ないと(?=いけ)|く(?=(\p{Han}|\p{Katakana}{2}|[0-9０-９]))|
    ほとんど|らしくて(?!は)|らしく(?!て)|ため([にの](?![はも])|ならば|なら(?!ば))|ため[にの][はも]|為に(?![はも])|為に[はも]|わけ(には|では|じゃ(?!あ))|ほうが(?=(\p{Han}|\p{Katakana}{2}|[0-9０-９]))|
    ...
    もなく(?!て)|ながら|がてら|った(?![らんのりわっがぞぜ])|よりも|かも(?=[しれ])|とともに(?![はも])|と共に(?![はも])|もっとも|すべて(?![でのを])|ただの|まま(?=(\p{Han}|\p{Katakana}{2}|[0-9０-９]))|
    ...
    \p{Han}し?い(?=(\p{Han}|\p{Katakana}{2}|[0-9０-９]))(?!出)|\p{Han}しく(?=(\p{Han}|\p{Katakana}{2}|[0-9０-９]))|べきじゃ(?!あ)|かなり(?=(\p{Han}|\p{Katakana}{2}|[0-9０-９]))|[えけげせぜてでねめれ]ば(?=(\p{Han}|\p{Katakana}{2}|[0-9０-９]))|
    ゆっくり(?=(\p{Han}|\p{Katakana}{2}|[0-9０-９]))|ちゃんと(?=(\p{Han}|\p{Katakana}{2}|[0-9０-９]))|(なければ|なきゃ|ないと)(?=(なら|いけ))|\p{Hiragana}(?=(はず|べき)(だ|よ|$|。|…|！|？))|\p{Hiragana}(?=\p{Katakana}{2})|て(?=ありがと)|
    なら(?=(\p{Han}|\p{Katakana}{2}|[0-9０-９]))|なのは|[えけげせぜてでねめれ][るてた](?=(\p{Han}|\p{Katakana}{2}|[0-9０-９]))|たく(?=な[いか])|[わかさたなまら]れ[るた](?=(\p{Han}|\p{Katakana}{2}|[0-9０-９]))|いくつか|\p{Han}ても|して(?=(\p{Han}|\p{Katakana}{2}|[0-9０-９]))|
    \p{Han}たる(?=(\p{Han}|\p{Katakana}{2}|[0-9０-９]))|という(?=(\p{Han}|\p{Katakana}{2}|[0-9０-９]))|を|な[くい](?=(\p{Han}|\p{Katakana}{2}|[0-9０-９]))|\p{Han}\p{Hiragana}に(?=な(る|った|らな))|いた(?=(\p{Han}|\p{Katakana}{2}|[0-9０-９]))|
    ないと(?=(\p{Han}|\p{Katakana}{2}|[0-9０-９]))|て(?=ほし[いくか])|\p{Han}{2}(?=\p{Katakana}{2})|な(?=(\p{Han}|\p{Katakana}{2}|[0-9０-９]))|\p{Katakana}{2}(?=\p{Han}{2})|(?P<doubler>\p{Hiragana}{2})(?P=doubler)|くて(?=\p{Han})|
    しか(?=(\p{Han}|\p{Katakana}{2}|[0-9０-９]))|よりかは|て(?=しま[ういわ])|とっ?ても|\p{Han}\p{Hiragana}(?=\p{Han}{2})|とか(?=\p{Han})|もう(?=\p{Han})|\p{Hiragana}(?=つもり)|が(?=(\p{Han}{2}|\p{Katakana}{2}))|
    なんて(?=こった)|による(?=(\p{Han}{2}|\p{Katakana}{2}))|[てで](?=しま)|[てで](?=お[かきくけこ])|何かが|[うくぐすずつぬむる]には
)
""", re.VERBOSE)
